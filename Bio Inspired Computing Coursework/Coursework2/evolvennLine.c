...
int COURSEWORK_MUTATE (void)
{
  ...
  int i, g, r;
  double gPerturbation1;
  double gPerturbation2;
  double gPerturbation3;
  int mutateNum = 3;
  ...
  gPerturbation1 = grand();
  gPerturbation2 = grand();
  gPerturbation3 = grand();

  // now mutate the child
  // Select 3 genes uniformly at random
  int mutateGenes[mutateNum];
  g = 0;
  while(g<mutateNum){
	r = rand()% numgenes;
	if(g==0){
	  mutateGenes[g]=r;
	  g += 1;
	}else if(g==1&&mutateGenes[0]!=r){
	  mutateGenes[g]=r;
	  g += 1;
	}else if(g==2&&mutateGenes[0]!=r&&mutateGenes[1]!=r){
	  mutateGenes[g]=r;
	  g += 1;
	}
  }
	child[mutateGenes[0]] += gPerturbation1;
	child[mutateGenes[1]] += gPerturbation1;
	child[mutateGenes[2]] += gPerturbation1;
...
int COURSEWORK_CROSSOVER (void)
{
  ...
  int i;
  // For Line Cross over, u is uniform random number between 0 and 1.
  // Only generate u one time and use it with every genes.
  double u = drand();
  
  // now make child[] using line crossover of parent1 and parent2
  for( i=0;i<numgenes; i++ )  
    {
	  child[i] = ( parent1[i] - alpha ) + u * ( parent2[i] - parent1[i] );
    }
